...
class AirtableServer:
    ...
    def __init__(self, api_key: str) -> None:
        ...
        application.router.add_routes((
            ...
            patch('/v0/{base_id}/{table_name}/{record_id}',
                  self.update_record),
            delete('/v0/{base_id}/{table_name}/{record_id}',
                   self.delete_record)
        ))
    ...
    def add_records(self, base_id: str, table_name: str,
                    records: List[aat.Record]) -> None:
        if (base_id, table_name) in self._tables:
            fields_keys = self._tables[base_id, table_name][0]['fields'].keys()
            assert all(fields_keys == record['fields'].keys()
    ...
    async def list_records(self, request: Request) -> Response:
        ...
        for sort_index in sorted(sort, reverse=True):
            ...
            records.sort(key=fields_sort_key(field),
        ...
        if 'fields[]' in request.query:
            ...
            for record in records:
                record['fields'] = {key: value for key, value in
                                    record['fields'].items() if key in fields}
    ...
    async def create_record(self, request: Request) -> Response:
        ...
        record = aat.Record(id=f'record{self._loop.time()}', fields=fields,
...
async def server(dt_str: str) -> AsyncGenerator[AirtableServer, None]:
    ...
    records = [
        ...
        }, createdTime=dt_str) for index in range(200)
    ]
...
def test_backoff_giveup(status: int) -> None:
    assert aat.backoff_giveup(client_response_error(status)) == (status != 429)
...
@pytest.mark.asyncio
async def test_airtable_request(server: AirtableServer,
...
async def test_airtable_base_repr(airtable: Airtable) -> None:
    base = airtable.base('some_base_id')
    assert repr(base) == ('AirtableBase(base_id=\'some_base_id\', '
...
@pytest.mark.asyncio
async def test_airtable_base_request(server: AirtableServer,
                                     airtable: Airtable, url: URL) -> None:
...
@pytest.mark.asyncio
async def test_airtable_table_request(server: AirtableServer,
                                      airtable: Airtable, url: URL) -> None:
    base = airtable.base('base_id')
...
@pytest.mark.asyncio
async def test_airtable_table_list_records(server: AirtableServer,
                                           airtable: Airtable,
                                           dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    ...
    assert server.requests() == [RequestData(
        'GET',
...
@pytest.mark.asyncio
async def test_airtable_table_iter_records(server: AirtableServer,
                                           airtable: Airtable,
                                           dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    records = [record async for record in table.iter_records(
...
@pytest.mark.asyncio
async def test_airtable_table_retrieve_record(server: AirtableServer,
                                              airtable: Airtable,
                                              dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    ...
    assert record.fields == {'field_1': 'value_1_003',
                             'field_2': 'value_2_003',
                             'field_3': 'value_3_003'}
    assert record.created_time == parse_dt(dt_str)
    assert record.table == table
...
@pytest.mark.asyncio
async def test_airtable_table_create_record(server: AirtableServer,
                                            airtable: Airtable,
                                            dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    ...
    assert record.fields == {'field_1': 'value_1_new_001',
                             'field_2': 'value_2_new_001',
                             'field_3': 'value_3_new_001'}
    assert record.created_time == parse_dt(dt_str)
    assert record.table == table
    assert server.requests() == [RequestData(
        'POST',
        table.url,
        {'fields': {'field_1': 'value_1_new_001',
                    'field_2': 'value_2_new_001',
                    'field_3': 'value_3_new_001'}}
    )]
...
@pytest.mark.asyncio
async def test_airtable_record_init_error(airtable: Airtable,
                                          string: str) -> None:
    base = airtable.base('some_base_id')
    table = base.table('some_table')
    with pytest.raises(ValueError, match='time data'):
...
async def test_airtable_record_repr(airtable: Airtable) -> None:
    ...
    assert repr(record) == ('AirtableRecord(record_id=\'record1\', fields={}, '
...
@pytest.mark.asyncio
async def test_airtable_record_request(server: AirtableServer,
                                       airtable: Airtable,
                                       url: URL, dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    record = AirtableRecord('record000', {}, dt_str, table)
    assert await record._request('GET', record.url) == aat.Record(
        id='record000',
        fields={'field_1': 'value_1_000',
                'field_2': 'value_2_000',
                'field_3': 'value_3_000'},
        createdTime=dt_str
    )
...
@pytest.mark.asyncio
async def test_airtable_record_update(server: AirtableServer,
                                      airtable: Airtable,
                                      dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    ...
    assert server.requests() == [RequestData(
        'PATCH',
        record.url,
        {'fields': {'field_1': 'value_1_new_000',
                    'field_2': 'value_2_new_000',
                    'field_3': 'value_3_new_000'}}
    )]
...
@pytest.mark.asyncio
async def test_airtable_record_delete(server: AirtableServer,
                                      airtable: Airtable,
                                      dt_str: str) -> None:
    base = airtable.base('base_id')
    table = base.table('table_name')
    ...
    with pytest.raises(RuntimeError, match='Record is deleted'):
        assert await record.update({'field_1': 'value_1_new_000',
                                    'field_2': 'value_2_new_000',
                                    'field_3': 'value_3_new_000'})
    assert server.requests() == [RequestData(
        'DELETE',
